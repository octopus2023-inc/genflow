...
class Visualizer:
    def __init__(self, yaml_file=None, functions_filename=None, structured_output_schema_filename=None):
        ...
        Args:
            ...
            functions_filename (str): Filename of the functions file (default: None).
            structured_output_schema_filename (str): Filename of the structured output schema file (default: None).
        """
        ...
        try:
            self.functions = importlib.import_module(self.functions_filename)
            logger.info(f"Imported functions from '{self.functions_filename}'")
        except ImportError as e:
            logger.error(f"Error importing functions file: {self.functions_filename}. {e}")
            self.functions = None

        try:
            self.structured_output_schema = importlib.import_module(
                self.structured_output_schema_filename)
            logger.info(f"Imported structured output schema from '{self.structured_output_schema_filename}'")
        except ImportError as e:
            logger.error(
                f"Error importing structured output schema file: {self.structured_output_schema_filename}. {e}")
                ...
                if node_type == 'function_call':
                    ...
                    if func_name:
                        if func_name.startswith('COMPOSIO.'):
                            ...
                            if func:
                                ...
                                content.append(html.P(f"Function '{func_name}' not found in '{self.functions_filename}'.", style={'color': '#fff'}))
                    else:
                    ...
                    if structured_output_schema_name:
                        ...
                        if schema:
                            ...
                            content.append(html.P(f"Schema '{structured_output_schema_name}' not found in '{self.structured_output_schema_filename}'.", style={'color': '#fff'}))
